...
function StackBar({
  ...
  title: string;
  note?: string;
  parts: { key: string; label: string; hours: number; cls: string }[];
  unit?: string;
}) {
  const total = parts.reduce((s, p) => s + p.hours, 0);
  const basis = total > 0 ? total : 1;
  return (
    <article className="icpdf-stack">
      <div className="icpdf-trend-head">
        <h4>{title}</h4>
        {note ? <code>{note}</code> : null}
      </div>
      <div className="icpdf-stack-bar">
        {parts.map((p) =>
          p.hours > 0 ? (
            <i key={p.key} className={p.cls} style={{ width: `${(p.hours / basis) * 100}%` }} />
          ) : null,
        )}
      </div>
      <p className="icpdf-stack-leg">
        {parts.map((p) => (
          <span key={p.key}>
            <i className={p.cls} /> {p.label} {n(p.hours, p.hours % 1 ? 1 : 0)} {unit}
            {total > 0 ? ` · ${((p.hours / total) * 100).toFixed(0)} %` : ""}
          </span>
        ))}
      </p>
    </article>
