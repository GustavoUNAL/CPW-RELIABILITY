...
/** 9 · Malos actores */
export function InformeConfMalosActoresSection({ month, monthLabel }: MonthProps) {
  return (
    <CollapsibleSlide
      n={9}
      title={`Malos actores · ${monthLabel}`}
      sub="Impacto operacional (IIO) por unidad"
      badge="GTE"
      className="inf-conf-slide-one"
    >
      {month === "Jul" ? (
        <div className="inf-conf-vs-box inf-conf-vs-box--compact">
          <p className="eyebrow">Julio vs junio</p>
          <p>
            Junio: {EXEC_JUN.failures} fallas imputables · foco {EXEC_JUN.focalUnit}. Julio: 0 FO
            imputables (Conf 100 %); las marcas de concertación no generan IIO de contratista.
          </p>
        </div>
      ) : null}
      <EventInsightsDashboard
  ...
  return (
    ...
    >
      <div className="inf-conf-vs-box inf-conf-vs-box--compact">
        ...
        <ul>
          <li>
            Disp. {(EXEC_JUN.availability * 100).toFixed(2)} % → 80,65 % (−17,27 pp)
          </li>
          <li>
            Conf. {(EXEC_JUN.reliability * 100).toFixed(2)} % → 100 % · fallas {EXEC_JUN.failures} → 0
          </li>
          <li>Presión julio: AGC4 (CPW01–03) y cascadas MRU sobre baseline junio</li>
        </ul>
      </div>
...
export function InformeConfContinuacion({ month, monthLabel }: MonthProps) {
  return (
    <>
      ...
      <InformeConfMalosActoresSection month={month} monthLabel={monthLabel} />
